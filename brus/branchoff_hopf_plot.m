...
proflab={{'UST',2},{'FP',1},{'UST',2}};
clri=[1,2,4];
figure(1);clf;tl=tiledlayout(3,2);
tl.TileSpacing='tight';
nexttile([3,1]);ax1=gca;hold(ax1,'on');
lw={'linewidth',2};
...
for i=1:nruns
    pl(i)=plot(ax1,hopf_eq{i}.lambda,hopf_eq{i}.amplitude,lw{:},'DisplayName',names{i},...
        'color',clr(clri(i),:));
    specsel{i}=ty(hopf_eq{i},'UST')|ty(hopf_eq{i},'BP')|ty(hopf_eq{i},'FP');
    uz=plot(ax1,hopf_eq{i}.lambda(specsel{i}),hopf_eq{i}.amplitude(specsel{i}),'ks',lw{:});
end
hbi=find(ty(hopf_eq{2},'EP')&hopf_eq{2}.amplitude<1e-2);
hb=plot(ax1,hopf_eq{2}.lambda(hbi),hopf_eq{2}.amplitude(hbi),'kd',lw{:},...
    'MarkerSize',12,'MarkerFaceColor','g','DisplayName','equivariant Hopf bifurcation');
...
for i=1:nruns
    ...
    pllab{i}=plot(ax(i),NaN,NaN,'o','MarkerEdgeColor','k',lw{:},...
        'MarkerFaceColor',clr(clri(i),:),'DisplayName',sprintf('$P_%d$',i));
    legend(ax(i),[pllab{i};tprofs{i}],txt{:},ltx{:},'Location','EastOutside');
